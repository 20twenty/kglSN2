...
RandomForest::RandomForest(xData* pXD, int fold, int folds, int Mtry, int trees, bool subset_on) : pXD_(pXD), fold_(fold), folds_(folds), 
   ...
   if(subset_on) {
      ...
      for(int f_idx = 0; f_idx < good_features_.size(); f_idx++) {
         ...
         if(good_features_[f_idx] >= pXD->attr_class.size()) {
            good_ = false;
            cout << "Good features specified are out of bounds" << endl;
            return;
         }
      }
   } else {
   ...
   cout << "good_features_cnt: " << good_features_.size() << endl;
   cout << "learning..." << endl;
...
void RandomForest::recAssignNode(Node* pNode, vector<pair<int,int> >& idx_orig, string pre, int depth, int& node_cnt, double& tree_gini) {
   ...
   while( best_ig <=0 && ntry < 1 ) {  // if you run into a feature with homogeneous values, try some times to avoid it
      ntry++;
      ...
      if(debug && 0) {
         ...
         for(int i=0; i<features.size(); i++) {
            if(features[i] == last) cout << endl << "ERROR: repeat feature" << endl;
            last = features[i];
            cout << features[i] << ",";
         }
         cout << endl;
   ...
   /*
   cout << pre << idx_orig.size() << "," << best_attr << "(" << pXD_->attr_class[best_attr] << ")," << best_ig << "," 
...
void RandomForest::recDelete(Node* pNode) {
   if(pNode->node0 != 0) {
      recDelete(pNode->node0);
      delete pNode->node0;
   }
   if(pNode->node1 != 0) {
      recDelete(pNode->node1);
      delete pNode->node1;
   }
   return;
}
...
void RandomForest::build() {
   ...
   while(1) {
      ...
      */
      ...
      cout << tree+1 << "/" << trees_ << "; M: " << base_feature_sample_cnt_ << "; bag_size: " << bag.size() \
      ...
      cout << "good_tree_cnt: " << good_tree_cnt << endl;
   ...
   if(!fOob.is_open()) {
      cout << "Cannot open " << oobFile << " for writing" << endl;
      return;
   }
