...
void xData::getMissing (int id, map<int,EdgeRec>& myMissing, ofstream& attrFile, ofstream& edgeFile, bool isTrain) {
   ...
   bool recommend_FRsFRs = false;
   bool recommend_recip = true;
   int voter_limit = 20;
   if(recommend_FRsFRs) {
      vector<Voter> voters;
      for(set<int>::iterator it_friend = graph_[id].friends.begin(); it_friend != graph_[id].friends.end(); it_friend++) {
         int myFR_id = *it_friend;
         Voter v;
         int ao_cnt=0, bo_cnt=0, ab_cnt=0;
         venn(graph_[id].friends, graph_[myFR_id].friends, ao_cnt, bo_cnt, ab_cnt, NULL, &(v.recommendations), NULL);
         v.recommendations.erase(id);  // need to remove myself
         bo_cnt--;
         if(bo_cnt > 0) {
            v.id=*it_friend;  // not needed for friends because I am already following this person
      ...
      for(vector<Voter>::iterator it_voter=voters.begin(); it_voter != voters.end(); it_voter++) {
         if(it_voter->recommendations.size() == 0) cerr << "This should not occur!";
         for(set<int>::iterator it_r=it_voter->recommendations.begin(); it_r!=it_voter->recommendations.end(); it_r++) {
            myMissing[*it_r].FRsFRs += it_voter->overlap / ((double)int_min(voter_limit,voters.size())*(double)it_voter->recommendations.size());
         }
         vR_count++;
         if(vR_count == voter_limit) break;
      }
   }
   if(recommend_LsFsLs) {
   ...
   for(map<int,EdgeRec>::iterator it_missing = myMissing.begin(); it_missing != myMissing.end(); it_missing++) {
      ...
      /*!!!*/ attrFile << it_missing->second.LsFs << "," 
                       << it_missing->second.LsFsLs << "," << it_missing->second.Recip << endl << flush; /*!!!*/
